...
function ModalMaker({ children, maxHeightAllocated = defaultHeight, maxWidthAllocated = defaultWidth, closeFunction = () => {}, show=false }) {
    return (
        <div className={show?"modal-container active":"modal-container"}>
            <div className={show?"card active":"card inactive"} style={{ maxHeight: maxHeightAllocated + 'px',  maxWidth: maxWidthAllocated + 'px'}}>
                {show && <div className="content">
                    {children}
                </div>}
                <button className="close-cross" onClick={closeFunction}>
